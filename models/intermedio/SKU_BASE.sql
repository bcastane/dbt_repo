...
WITH CURRENT_DATA AS (SELECT scraped_at,scraped_at_date,sku,retail_id,product_name,product_url,offer_price,normal_price,card_price,loyalty_price,picture_url ,CASE WHEN brand is null THEN 'sin marca' ELSE lower(brand) END AS brand,rating,n_reviews,pos,  ROW_NUMBER() OVER (PARTITION BY sku,retail_id ORDER BY scraped_at DESC) as N
